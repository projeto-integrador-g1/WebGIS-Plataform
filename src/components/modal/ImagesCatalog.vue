    ...
    <v-row justify="center">
      <v-dialog v-model="showImagesCatalog" scrollable max-width="800px">
        <v-card height="100%">
          ...
          <v-divider></v-divider>
          <v-row
            v-show="Object.keys(imagesCatalog).length == 0"
            justify="center"
            style="height: 600px;"
          >
          ...
          <v-row v-show="Object.keys(imagesCatalog).length > 0">
            <v-col
              v-for="(image, index) in imagesCatalog"
              :key="index"
              cols="6"
              sm="4"
            >
          ...
          <v-divider></v-divider>
          <v-card-actions>
            <v-btn color="blue darken-1" text @click="closeImagesCatalog"
              >Cancelar</v-btn
            >
            <v-btn color="blue darken-1" text @click="validateImagesCatalog"
              >Pesquisar</v-btn
            >
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
    validateImagesCatalog() {
      ...
      console.log(
        "Catalogo de imagens escolhido: " + this.selectedImagesCatalog
      );
      this.catalog.images = [];
